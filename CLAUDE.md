...
- Do not introduce new `AGENT_SANDBOX_*` environment variable flags to gate features. New functionality in `start.sh` should be installed unconditionally (with idempotency guards like sentinel files) and rely on `|| true` or `if/else` to handle failures gracefully without blocking container startup.
